...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
#define PRINCIPAL_FREQUENCY 0x00ffffff
#define PRESCALE 0x0000ffff
#define TICKS_IN_SEC (PRINCIPAL_FREQUENCY / PRESCALE)
#define FLICKS_IN_SEC 4
...
int main(void)
{
  ...
  // Turn on TIM6 clock enable and GPIOD enable.
  ...
  *(tim6_periph + TIM6_CR1_OFFSET) |= 0x00000001;
  ...
  uint32_t* prescaler = tim6_periph + TIM6_PSC_OFFSET;
  *(tim6_periph + TIM6_PSC_OFFSET) |= PRESCALE;

  uint32_t last_counter = *(tim6_periph + TIM6_CNT_OFFSET);
  /* USER CODE END 2 */
  ...
  *(gpiod_periph + GPIOD_ODR_OFFSET) |= 0x0000f000;
  while (1)
  {
    ...
    uint32_t local_counter = *(tim6_periph + TIM6_CNT_OFFSET);
    if (local_counter - last_counter >= TICKS_IN_SEC / FLICKS_IN_SEC){
      last_counter = local_counter;
      if(is_on){
        is_on = false;
